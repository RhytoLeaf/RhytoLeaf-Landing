// Zeek RSVP Speed Reader — Reader Window Controller (Safari)
...
function loadAndStartReader(text) {
  ...
  browser.storage.session.set({ raw_text: text });
...
async function scanText() {
  ...
  try {
    ...
    if (typeof Tesseract === 'undefined') {
      await loadScript(browser.runtime.getURL('lib/tesseract/tesseract.min.js'));
    }

    const workerPath = browser.runtime.getURL('lib/tesseract/tesseract-worker.min.js');
...
async function checkPendingText() {
  try {
    const { pending_text } = await browser.storage.session.get('pending_text');
    if (pending_text) {
      await browser.storage.session.remove('pending_text');
...
async function checkResumableState() {
  try {
    const { reading_state, raw_text } = await browser.storage.session.get(['reading_state', 'raw_text']);
...
// Listen for new pending text while reader window is open
browser.storage.session.onChanged.addListener((changes) => {
  if (changes.pending_text?.newValue) {
    browser.storage.session.remove('pending_text');
    loadAndStartReader(changes.pending_text.newValue);
  }
});

// Save state when window becomes hidden
...
async function init() {
  ...
  dom.pasteBtn.addEventListener('click', async () => {
    try {
      ...
      dom.textInput.focus();
      alert('Clipboard access denied. Please use Cmd+V to paste.');
    }
  });
